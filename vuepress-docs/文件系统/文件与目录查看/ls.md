...
-G, --no-group
	在长格式(-l)输出时，不输出组名。
-h, --human-readable
	与 -l 和/或 -s 一起使用，打印人类可读的大小（例如 1K 234M 2G）
--si
...
ls -t
```
如果想按照修改日期递增，加上选项 -r 即可。

（4）按照文件大小降序排列。
```bash
ls -lhS
```
如果想按照文件大小升序排列，加上选项 -r 即可。

（5）查看文件或目录索引节点号。

在 Linux 文件系统中，每个文件和目录都有一个唯一的标识号，称为索引节点号（inode number 或 inode）。索引节点包含了关于文件或目录的元数据信息，但不包括文件名和实际数据。

每个文件系统都有一个 inode 表，inode 表记录了所有文件和目录的 inode 号码以及它们的相关信息。在文件系统初始化时，inode 表被创建，每个文件或目录被分配一个唯一的 inode 号码。

一个 inode 包含的元数据信息通常包括：

- 文件类型： 指明是文件、目录、符号链接等。
- 文件权限： 包括文件所有者、文件所属组以及其他用户的权限信息。
- 文件大小： 记录了文件实际数据所占用的空间大小。
- 时间戳： 记录了文件的访问时间、修改时间和inode修改时间。
- 硬链接计数： 记录有多少硬链接指向该 inode。
- 指向文件数据的指针： 指向文件数据的物理位置。

inode 的使用对于文件系统的性能和管理都非常重要。文件名和实际数据在文件系统中存储的位置会引用 inode 号码，这样文件系统可以直接查找相应的 inode 来获取文件或目录的元数据信息，然后再访问实际数据。这样的设计可以提高文件系统的效率，特别是在处理大量小文件的情况下。

你可以使用 ls -i 命令来查看文件或目录的索引节点号：
```shell
ls -i FILE
```
